...
import { Truck, MapPin, Camera, PackageCheck, ChevronRight } from 'lucide-react'
  ...
  return (
    <Card className="space-y-3">
      <div className="flex items-start justify-between gap-2">
        <button
          onClick={() => navigate(`/orders/${assignment.order_id}`)}
          className="group -m-1 flex items-start gap-1 rounded-[var(--radius-sm)] p-1 text-left transition-colors hover:bg-surface-2"
        ...
        >
          ...
          <ChevronRight size={14} className="mt-0.5 text-muted opacity-0 transition-opacity group-hover:opacity-100" />
        </button>
        <span
